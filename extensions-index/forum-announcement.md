...
feedback. Edit to taste, then post — it's your voice, so make it yours.

---

**Title:** New extension: SlicerForge (DICOM Forge Batch) — batch de-identify, QC & import DICOM
...
pipeline: **ingest → de-identify → quality-control → convert (NRRD/NIfTI) → load
...
- The heavy lifting lives in a **separate, headless engine** (`dicom-anvil` on
  ...
  the Slicer module stays a thin GUI — the same ITK/VTK-style split Slicer uses.
- CI runs the **full pipeline in a real headless Slicer** on every push, plus the
  engine's own test matrix.
- It's **open-core**: the Slicer extension is Apache-2.0; the engine is PolyForm
  Noncommercial (free for research/education).

Repo: https://github.com/DaCameraGirl/slicer-forge
Engine: https://github.com/DaCameraGirl/dicom-forge · https://pypi.org/project/dicom-anvil/
